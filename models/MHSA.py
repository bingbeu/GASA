...
class DynamicChannelAdapter(nn.Module):
    def __init__(self, out_channels=16, kernel_size=1):
        super().__init__()
        self.out_channels = out_channels
        self.kernel_size = kernel_size

    def forward(self, x):
        B, C, L = x.shape  # x shape: (B, C, L)

        conv = nn.Conv1d(in_channels=C, out_channels=self.out_channels,
                         kernel_size=self.kernel_size, device=x.device, dtype=x.dtype)

        x = conv(x)  # (B, out_channels, L)
        return x
...
class MHSABlock(nn.Module):
    def __init__(self, input_dim, output_dim,image_size, stride, num_heads,extra_token_num=1,mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, conv1d_kernel_size=3):
        super().__init__()
        if stride != 1:
        ...
        self.grad_proj = nn.Linear(2 * output_dim, output_dim)
        # 3) Sobel
        sobel_kernel = torch.tensor([[1, 0, -1],
                                     [2, 0, -2],
                                     [1, 0, -1]],
                                    dtype=torch.float32)
        self.register_buffer('sobel_kernel_x', sobel_kernel.view(1,1,3,3))
        self.register_buffer('sobel_kernel_y', sobel_kernel.view(1,1,3,3).transpose(-1, -2))
        self.channel = DynamicChannelAdapter(out_channels=16, kernel_size=1)
        # ---- 新增：H、W 方向的 1D 卷积 ----
        ks = conv1d_kernel_size
        ps = ks // 2
        # 注意 Conv1d 的输入 / 输出都是“channel first”，长度是 H 或 W
        self.conv_h = nn.Conv1d(in_channels=2*output_dim,  # 这里假设最终通道数是 16
                                out_channels=2*output_dim,
                                kernel_size=ks,
                                padding=ps,
                                bias=False)
        self.conv_w = nn.Conv1d(in_channels=2*output_dim,
                                out_channels=2*output_dim,
                                kernel_size=ks,
                                padding=ps,
                                bias=False)
    def forward(self, x, H, W, extra_tokens=None,):
        ...
        if self.patch_embed is not None:
            ...
            y = x.reshape(B, Hf, Wf, C).permute(0, 3, 1, 2)  # -> (B, C, Hf, Wf)
            Gx = F.conv2d(
                y,
                self.sobel_kernel_x.repeat(C, 1, 1, 1),
                padding=1,
                groups=C
            )  # [B, C, Hf, Wf]
            Gy = F.conv2d(
            ...
            grad_map = torch.cat([Gx, Gy], dim=1)
            avg_H = F.adaptive_avg_pool2d(grad_map, (Hf, 1)).squeeze(-1)
            #  [B, C, 1, Wf] → squeeze → [B, C, Wf]
            avg_W = F.adaptive_avg_pool2d(grad_map, (1, Wf)).squeeze(-2)

            # 
            #    
            attn_H = torch.sigmoid(self.conv_h(avg_H))  # → [B, C, Hf]
            attn_W = torch.sigmoid(self.conv_w(avg_W))  # → [B, C, Wf]

            #  [B, C, Hf, 1] * [B, C, 1, Wf] → [B, C, Hf, Wf]
            attn_2d = attn_H.unsqueeze(-1) * attn_W.unsqueeze(-2)
            grad_map = grad_map * attn_2d
            N = Hf * Wf
            grad_flat = grad_map.view(B, 2 * C, N).transpose(1, 2) 
            grad_tokens = self.grad_proj(grad_flat)
            grad_tokens = self.channel(grad_tokens)
            x = torch.cat([grad_tokens, x], dim=1)
